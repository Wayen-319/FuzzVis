...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  fprintf(log, "\n[MUTATE STAGE]:\n%s\n", afl->stage_short);
  fprintf(log, "\n[OUTPUT SEED afl-fuzz-bitmap.c]:\n");
  fwrite((char *)mem, 1, len, log);
  fflush(log);
  fclose(log);
  ...
  save_to_queue:
    ...
    if (likely(fd >= 0)) {
      ...
      const char *base = strrchr(queue_fn, '/');
      if (base) base++; else base = queue_fn;
      snprintf(op_fn, PATH_MAX, "/home/spark/AFLplusplus/tmp/%s.txt", base);
      
      FILE *oplog = fopen(op_fn, "a+");

      if (oplog){
         fprintf(oplog, "%s\n", afl->mutation_history);
         fflush(oplog);
	 fclose(oplog);
      }
